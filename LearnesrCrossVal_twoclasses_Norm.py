...
files=sorted(glob.glob('/home/ricardo/Documentos/proyectoSPIE/featuresGlandCOlon/featuresNorm_*filterNan.mat'), key=os.path.basename)
...
for fileTrain in files:
    ...
    Xtrain = file_mat['all_features']
    Ytrain = file_mat['all_labels']

    #search and eliminate Nan rows
    RowsNan = []
    for n in range(0, Xtrain.shape[0]):
        if np.isnan(Xtrain[n, :]).any() == True:
            RowsNan.append(n)
            print('value:', n, np.isnan(Xtrain[n, :]).any() == True)
    ...
    for z in range(len(Name_test)):
        ...
        for name, clf in zip(names, classifiers):
          #  ax = plt.subplot(len(datasets), len(classifiers) + 1, i)
            ...
            scores = cross_val_score(clf, X_train, y_train, cv=5,scoring='roc_auc',error_score=np.nan)
            score = scores.mean()
            print(name,': ',score)
