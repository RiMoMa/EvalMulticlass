...
files=sorted(glob.glob('/home/ricardo/Documents/Doctorado/CreacionArticulos/articuloPNWithAnne/borrar/train*.mat'), key=os.path.basename)

import scipy.io as sio
from sklearn.model_selection import cross_val_score #pruebaV3Test_grafo.npy
Kfold = 10
FileResults='/home/ricardo/Documents/Doctorado/SYNC/ResultadosNPN/prueba_grafo_Partition_haralickMeans.npy'#'/home/ricardo/Documents/Doctorado/SYNC/ResultadosNPN/prueba_grafo_Partition_haralickMeansV2GoodImageOut.npy'#prueba_grafo_Partition_haralickMeans.npy'
...
#        fileTE = fileTrain[0:57]+'test'+fileTrain[62::]
        fileTE = fileTrain[0:78] + 'test' + fileTrain[83::]
...
idxF = np.where(all_train_val[:,16]==np.float(lista_train[2,16]))#16
idxF=np.int(idxF[0][0])
valuesT = ScoresLearners_filesTrain[idxF]
y_score = concatenateKFold(ScoresLearners_filesTrain[idxF],16)
y_test = concatenateKFold(LabelsLearners_filesTrain[idxF], 16)
y_pred3Vall = y_score>0.35

y_scoreTest = concatenateKFold(ScoresLearners_filesTest[idxF],16)
y_testTest = concatenateKFold(LabelsLearners_filesTest[idxF], 16)
y_pred3VallTest = y_scoreTest>0.35



idxF = np.where(all_train_val[:,70]==np.float(lista_train[2,70]))#70
idxF=np.int(idxF[0][0])
valuesT = ScoresLearners_filesTrain[idxF]
y_score1V2 = concatenateKFold(ScoresLearners_filesTrain[idxF],130)#130
y_test1V2 = concatenateKFold(LabelsLearners_filesTrain[idxF], 130)
y_pred1V2 = y_score1V2<(1-0.487)

y_score1V2Test = concatenateKFold(ScoresLearners_filesTest[idxF],130)
y_test1V2Test = concatenateKFold(LabelsLearners_filesTest[idxF], 130)
y_pred1V2Test = y_score1V2Test<(1-0.487)
...
allLabels = concatenateKFold(LabelsLearners_filesTrain[4],7)
...
allLabelsTest = LabelsLearners_filesTest[6][4][7]
