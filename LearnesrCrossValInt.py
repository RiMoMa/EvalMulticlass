...
files=sorted(glob.glob('/home/ricardo/Documentos/experimentosMay/serverPF/train*.mat'), key=os.path.basename)
...
for fileTrain in files:
    ...
    for z in range(len(Name_test)):
        ...
        if z==0:
            ...
            Ytotal = np.concatenate((Ytrain1*0, Ytrain2K * 0, Ytrain3/3), axis=0)
            Xtesttotal = np.concatenate((Xtest1, Xtest2, Xtest3), axis=0)
            Ytesttotal = np.concatenate((Ytest1*0, Ytest2 * 0, Ytest3/3 ), axis=0)
            ...
            Ytotal = np.concatenate((Ytrain1 ,Ytrain2K*0), axis=0)
            ...
            Ytotal = np.concatenate((Ytrain1, Ytrain3*0), axis=0)
            Xtesttotal = np.concatenate((Xtest1, Xtest3), axis=0)
            Ytesttotal = np.concatenate((Ytest1, Ytest3*0), axis=0)
        elif z == 4:
            Xtotal = np.concatenate((Xtrain3, kmeans), axis=0)
            Ytotal = np.concatenate((Ytrain3*0, Ytrain2K/2), axis=0)
            Xtesttotal = np.concatenate((Xtest3, Xtest2), axis=0)
            Ytesttotal = np.concatenate((Ytest3*0, Ytest2/2), axis=0)
        elif z == 5:
            Xtotal = np.concatenate((Xtrain1, kmeans, Xtrain3), axis=0)
            Ytotal = np.concatenate((Ytrain1*0, Ytrain2K/2 , Ytrain3*0), axis=0)
            Xtesttotal = np.concatenate((Xtest1, Xtest2, Xtest3), axis=0)
            Ytesttotal = np.concatenate((Ytest1*0, Ytest2 /2, Ytest3*0 ), axis=0)
